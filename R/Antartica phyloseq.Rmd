...
class_selected <- c("Dictyochophyceae", "Chrysophyceae", "Pelagophyceae", "Bacillariophyta", "Cryptophyceae", "Pyramimonadales", "Mamiellophyceae")
  ...
  for (one_sample_type in sample_type_surface[4:11]) { 
   ...
   gg  <- plot_heatmap(ps_heat, method = "NMDS", distance = "bray", 
                       ...
                       scale_fill_gradient(limits= c(0,30000), low="beige", high="red", na.value="gray95")
...
species_selected <- c("Thalassiosira_minima", "Fragilariopsis_cylindrus",  "Minidiscus_sp.", "Chaetoceros_neogracilis",
  ...
  for (one_sample_type in sample_type_surface[4:11]) { 
    ...
    gg  <- plot_heatmap(ps_heat, method = "NMDS", distance = "bray", 
                       ...
                       xlab("") + ylab("") +
                       theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust=1))+
                       scale_fill_gradient(limits= c(0,20000), low="beige", high="red", na.value="gray95")
...
long_class_sample <- list()
  ...
  for (one_sample_type in sample_type) {
      ...
      long_one <- phyloseq_transform_to_long(ps_one) %>%
        group_by(date, station_id, depth, fraction_name, division, class, order, family, genus) %>%
        summarize(n_reads = sum(n_reads)) %>% 
        ungroup()
      
      long_genus_sample[[one_sample_type]] <- long_one 
 ...
 long_compare_genus_18S_16S <- full_join(rename(long_genus_sample[["18S filter"]], n_reads_18S_filter = n_reads),
                                           rename(long_genus_sample[["16S plastid"]],n_reads_16S_plastid = n_reads))
 
 ggplot(long_compare_genus_18S_16S, aes(x = n_reads_18S_filter, y = n_reads_16S_plastid)) +
   geom_point(aes(color=class)) +
   scale_color_manual(values = class_colors)
 
 long_compare_class_18S_16S <- long_compare_genus_18S_16S %>% 
        group_by(date, station_id, depth, fraction_name, division, class) %>%
...
long_class   <- list()
  ...
  for (one_sample_type in sample_type) {
      long_one <- long_genus_sample[[one_sample_type]] %>%
...
long_genus_upset <- long_genus_comparison %>% 
  select(genus, contains("samples")) %>%
  mutate_at(vars(contains("n_")), funs(case_when(is.na(.) ~ 0,
                                                  TRUE ~ 1))) %>% 
  rename_at(vars(contains("n_")), funs(str_replace(.,"n_samples_", ""))) %>% 
  as.data.frame()

upset(long_genus_upset, empty.intersections = "on", number.angles = 30, point.size = 3.5, line.size = 2, 
    mainbar.y.label = "Genus Intersections", sets.x.label = "Genus per sample type", 
...
long_class_fraction <- list()
  ...
  for (one_sample_type in sample_type[c(1,3)]) {
      long_one <- long_genus_sample[[one_sample_type]] %>%
        filter(date >= as.Date("2015-01-01")) %>% 
        filter(!str_detect(genus, "_X")) %>%  
        group_by(fraction_name, division, class, order, family, genus) %>%
  ...
  long_class_fraction_comparison <- long_class_fraction %>% 
    purrr::reduce(full_join)
  long_genus_fraction_comparison <- long_genus_fraction %>% 
    purrr::reduce(full_join)
...
long_genus_upset <- long_genus_fraction_comparison %>% 
  mutate_if(is.numeric, funs(case_when(is.na(.) ~ 0,
                                                  TRUE ~ 1))) %>% 
  as.data.frame()

upset(long_genus_upset, empty.intersections = "on", number.angles = 30, point.size = 3.5, line.size = 2, 
    mainbar.y.label = "Genus Intersections", sets.x.label = "Genus per sample type", order.by = "freq",
    text.scale = c(1.3, 1.3, 1, 1, 2, 1.5))
